...
#include <iostream>
using namespace std;	//TODO only for logging
...
AD9954::AD9954(byte ssPin, byte resetPin, byte updatePin, byte ps0, byte ps1, byte osk)
{
   ...
   int fd = wiringPiSPISetup(CHANNEL, 5000000);
...
AD9954::AD9954(byte resetPin, byte updatePin)
{
	wiringPiSetup ();
	cout << "Initializing" << endl ;
   // Configure the interface.
   ...
   cout << "Init result: " << fd << endl;
    ...
    digitalWrite(_resetPin, LOW);
    digitalWrite(_updatePin, LOW);
	   cout << "_resetPin " << int(_resetPin) << ", _updatePin" << int(_updatePin) << " setup complete" <<endl;
...
void AD9954::initialize(unsigned long refClk){
    ...
    AD9954::writeRegister(registerInfo, 4, data);
...
void AD9954::initialize(unsigned long refClk, byte clkMult){
    ...
    if (_refClk < 25000000){ 
        ...
        AD9954::writeRegister(registerInfo, 3, data);
    }
}
...
void AD9954::setFreq(unsigned long freq){
    ...
    AD9954::writeRegister(CFR1Info, 4, CFR1);
	AD9954::writeRegister(registerInfo, 4, ftw);
...
void AD9954::linearSweep(unsigned long freq0, unsigned long freq1, unsigned long posDF, byte posRR, unsigned long negDF, byte negRR){
    ...
    AD9954::writeRegister(CFR1Info, 4, CFR1);
    AD9954::writeRegister(FTW0Info, 4, FTW0);
    AD9954::writeRegister(FTW1Info, 4, FTW1);
    AD9954::writeRegister(NLSCWInfo, 5, NLSCW);
    AD9954::writeRegister(PLSCWInfo, 5, PLSCW);
...
void AD9954::writeRegister(byte registerInfo[], int len, byte data[]){
    //digitalWrite(_ssPin, LOW); //TODO check not needed, done by SPI (/CS)
