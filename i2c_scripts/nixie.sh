...
# odr = 0
# intpol = 0
#i2cset -y 2 0x21 0x05 0x00
#verify config reg.
#i2cget -y 2 0x21 0x05

# set(IODIR_A, all as outputs (0))
i2cset -y 2 0x21 0x00 0x00
i2cset -y 2 0x20 0x00 0x00

# set(IODIR_B, all as outputs (0))
i2cset -y 2 0x21 0x01 0x00
...
i2cset -y 2 0x21 0x12 0x14

#set "56" on next 2 tubes
i2cset -y 2 0x21 0x13 0x02

#set "78" on next 2 tubes
i2cset -y 2 0x20 0x12 0x75

#set on last tube, 0 == no dot marker above any digit
i2cset -y 2 0x20 0x13 0x20

while :; do i2cset -y 2 0x20 0x13 0x10; sleep 1; i2cset -y 2 0x20 0x13 0x14; sleep 1; i2cset -y 2 0x20 0x13 0x10; sleep 1; i2cset -y 2 0x20 0x13 0x16; sleep 1; done
